...
exports.expectSamples = function(onContext, expected, done) {
  ...
  renderSamples(channelCount, frameCount, onContext, function(err, actual) {
    if (err) return done(err)
    try { assertBlocksEqual(actual, expected) } catch(err) { return done(err) }
    done()
  })
}

// Renders on audio block of size (`channelCount`, `frameCount`), and calls `done(err, block)`
var renderSamples = exports.renderSamples = function(channelCount, frameCount, onContext, done) {
  var context = new OfflineAudioContext(channelCount, frameCount, 44100)
  onContext(context)
